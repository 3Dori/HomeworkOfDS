...
typedef struct{
  ...
  int dist;
  int path;
  int count;    // Note: an extra attribute count is added
} Info;
...
Info *create_table(int size){
  ...
  for (int i = 1; i <= size; i++){
    ...
    table[i].count = 0;    // Note: the extra attribute count is set to 0
  }
...
Info *dijkstra(Graph G, int from){
  ...
  table[from].dist = 0;
  table[from].count = 1;    // Note: the extra attribute count is set to an initial value 1
  for (;;){
    smallest_dist = INF;
    has_unknown = FALSE;
    for (int i = 1; i <= G->vertex_num; i++){
	...
	if (table[smallest_unknown].dist + adjacent->weight < table[adjacent->index].dist){
	  ...
	  table[adjacent->index].count = table[smallest_unknown].count;
	}
	else if (table[smallest_unknown].dist + adjacent->weight == table[adjacent->index].dist){
	  table[adjacent->index].count += table[smallest_unknown].count;    // If a different minimum path appears, increase the count
	}
      }
    }
  }
